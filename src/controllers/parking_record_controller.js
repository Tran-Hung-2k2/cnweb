...
const controller = {
    ...
    async get_all_parking_record(req, res) {
        try {
            ...
            return res
                .status(200)
                .json(api_response(false, 'Lấy danh sách thông tin gửi xe thành công', parking_records));
        } catch (error) {
            return res.status(500).json(api_response(true, 'Lấy danh sách thông tin gửi xe thất bại'));
        }
    },
    ...
    async add_parking_record(req, res) {
        try {
            ...
            return res.status(500).json(api_response(true, 'Thêm thông tin gửi xe mới thất bại'));
        }
    },
};
